# ブジネスク解（全変位のノルム |u| = sqrt(u_r^2 + u_z^2)）
...
# 3. 全変位ノルム |u| の計算用配列の初期化 (初期値は NaN)
...
u_r_SI = (P / (4 * np.pi * G * R_SI)) * ((r_SI * Z_SI / R_SI**2) - ((1 - 2 * nu) * r_SI / (R_SI + Z_SI)))

# 【変更点】ノルム（三平方の定理）を計算し、nm 単位に換算して格納
...
# 等高線プロット
contour = ax.contourf(R_mesh, Z_mesh, u_norm_nm, levels=levels_setting, cmap='viridis', extend='max', zorder=1)
cbar = fig.colorbar(contour, ax=ax)
cbar.set_label('Total Displacement Norm, $|u|$ /nm', fontsize=12)
...
# 軸の設定
ax.set_title('Boussinesq Solution (displacement norm, $|u|$)', fontsize=14, pad=15)
