...
# 3. 応力 σ_r の計算用配列の初期化 (初期値は NaN)
sigma_r_Pa = np.full_like(Z_mesh, np.nan)
...
# σ_r の計算（合意通りの符号：圧縮が負、引張が正）
# 公式: sigma_r = (P / (2*pi*R^2)) * [ -3*r^2*z/R^3 + (1-2*nu)/(1 + z/R) ]
sigma_r_SI = (P / (2 * np.pi * R_SI**2)) * (
    -(3 * r_SI**2 * Z_SI / R_SI**3) + (1 - 2 * nu) * (1.0 / (1.0 + Z_SI / R_SI))
)

# パスカル [Pa] の単位のまま格納
sigma_r_Pa[valid_idx] = sigma_r_SI

# 5. 可視化
fig, ax = plt.subplots(figsize=(8, 6))

# 応力のプラス（引張）とマイナス（圧縮）を両方表現するため、0を中心とした対称な表示範囲にする
vmax_val = 1.05e6  # 50 kPa までをグラデーションで表現（お好みで調整してください）
levels_setting = np.linspace(-vmax_val, vmax_val, 51)

# 等高線プロット (中央の0が白、引張[正]が赤、圧縮[負]が青になる 'RdBu_r' を使用)
contour = ax.contourf(R_mesh, Z_mesh, sigma_r_Pa, levels=levels_setting, cmap='RdBu_r', extend='both', zorder=1)
cbar = fig.colorbar(contour, ax=ax)
cbar.set_label('Radial Stress, $\sigma_r$ / Pa', fontsize=12)
...
# 軸の設定
ax.set_title('Boussinesq Solution (stress, $\sigma_r$)', fontsize=14, pad=15)
...
fig.savefig('./png/Boussinesq_sr.png', dpi=300)
